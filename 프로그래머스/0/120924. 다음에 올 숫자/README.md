...
### 풀이
등차수열 아니면 등비수열이므로 if 조건문을 통해 등차수열일 때 index 기준 0번과 끝번의 합 = 1번과 끝-1번의 합이 같다는 것을 이용해 답을 쓰고
등비수열일때도 idx가 커질때마다 곱해지는 값이 같으므로 answer = 끝번 인덱스의 값 * 1번 인덱스의 값 / 0번 인덱스의 값을
else에 별다른 조건 없이 넣어주면 된다.
